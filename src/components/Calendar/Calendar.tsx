...
const total =
  dummyData.datasets[0].data[0] + dummyData.datasets[0].data[1] + dummyData.datasets[0].data[2]
...
const Calendar = () => {
  ...
  const renderPopoverContent = (date: Date, event: Event | undefined) => {
    ...
    if (event) {
      const eventsWithSameDay = events.filter((eventItem) => eventItem.date === event.date)
      return (
      ...
      return (
        <Popover id={'popover-empty'}>
          <Popover.Header as='h3'>
            {date.toLocaleString('default', { month: 'long' })} {dayOfMonth}, {date.getFullYear()}{' '}
          </Popover.Header>
                      ...
                      {Array.from({ length: 7 }, (_, j) => {
                        ...
                        return (
                          <td
                            ...
                            } ${isSelectedDate && eventForDate ? 'selected-date-with-events' : ''}  ${
                              isCurrentDate && isSelectedDate ? 'selected-current-date' : ''
                            }`}
                            key={dateIndex}
          ...
          <div className='EventsAttendanceCol'>
            ...
            {eventsday.length > 0 ? (
              <div className='mt-3 px-5'>
                ...
                <select id='eventDropdown' onChange={handleDropdownChange} value={selectedEvent}>
              ...
              <div className='mt-3 px-5'>
                <label htmlFor='eventDropdown' style={{ marginRight: '10px' }}></label>
              </div>
            )}
            <Container
              fluid
              style={{ maxWidth: '710px', maxHeight: '412px', borderRadius: '20px' }}
              className='px-5 py-3'
            >
              <Row>
                <Col xs={6}>
                              ...
                              ? {
                                  data: [
                                    Math.round(
                                      eventsday.reduce(
                                        (total, e) =>
                                          total + (e.title === selectedEvent ? e.attendees : 0),
                                        0
                                      )
                                    ),
                                    Math.round(
                                      eventsday.reduce(
                                        (total, e) =>
                                          total + (e.title === selectedEvent ? e.didNotAttend : 0),
                                        0
                                      )
                                    ),
                                    Math.round(
                                      eventsday.reduce(
                                        (total, e) =>
                                          total +
                                          (e.title === selectedEvent
                                            ? e.numberOfInviteSent - e.registered
                                            : 0),
                                        0
                                      )
                                    ),
                ...
                <Col>
                  <Button style={viewDetailsButton} onClick={handleUnavailableLinkClick}>
                    View details{' '}
                    <img
                      style={{ height: '12px', width: '13px', marginLeft: '5px', marginBottom: '3px' }}
                  ...
                  <Container fluid className='mt-5 px-1 py-2'>
                    <Row className='pt-4' style={{ alignItems: 'center' }}>
                      <div style={legendCircle1}></div>
                      {
                        <p
                          className='mb-0'
                          style={{ width: 'max-content', fontSize: '16px', display: 'inline-block' }}
                        >
                          Attendees <span style={{ color: 'lightgrey' }}>_____________</span>{' '}
                          {Math.round(
                            eventsday.reduce(
                              (total, e) =>
                                total +
                                (e.title === selectedEvent
                                  ? (e.attendees / e.numberOfInviteSent) * 100
                                  : 0),
                              0
                            )
                          )}
                    ...
                    <Row className='mt-3' style={{ alignItems: 'center' }}>
                      <div style={legendCircle2}></div>
                      {
                        <p
                          ...
                          style={{ width: 'max-content', fontSize: '16px', display: 'inline-block' }}
                        >
                          Didn't Attend <span style={{ color: 'lightgrey' }}>___________</span>
                          {Math.round(
                            eventsday.reduce(
                              (total, e) =>
                                total +
                                (e.title === selectedEvent
                                  ? (e.didNotAttend / e.numberOfInviteSent) * 100
                                  : 0),
                              0
                            )
                          )}
                    ...
                    <Row className='mt-3' style={{ alignItems: 'center' }}>
                      <div style={legendCircle3}></div>
                      {
                        <p
                          ...
                          style={{ width: 'max-content', fontSize: '16px', display: 'inline-block' }}
                        >
                          No Response <span style={{ color: 'lightgrey' }}>___________</span>
                          {Math.round(
                            eventsday.reduce(
                              (total, e) =>
                                total +
                                (e.title === selectedEvent
                                  ? ((e.numberOfInviteSent - e.registered) / e.numberOfInviteSent) * 100
                                  : 0),
                              0
                            )
                          )}
        ...
        <div style={{ backgroundColor: '#FFFF', padding: '12px', borderRadius: '20px' }}>
          <Container fluid style={{ margin: '0', padding: '0', backgroundColor: '#FFFF' }}>
            <Row style={{ paddingTop: '21px' }}>
              ...
              <Col>
                <div style={{ display: 'inline-block', float: 'right', paddingLeft: '20px' }}>
                  <Button style={viewDetailsButton} onClick={handleUnavailableLinkClick}>
                    Generate Report
                    <img
                      style={{ height: '12px', width: '13px', marginLeft: '5px', marginBottom: '3px' }}
                  ...
                  <FeatureUnavailable
                    show={showFeatureUnavailable}
                    onHide={handleCloseFeatureUnavailable}
                  />{' '}
                </div>
                <div
                  style={{
                ...
                >
                  <p>
                    {' '}
                    <img
                      className='EventReportCircle'
                      src={require('../../assets/images/greyIcon.png')}
                    />{' '}
                    Didn't Attend{' '}
                  </p>
                </div>
            ...
            <Row style={TitleBar} className='px-3'>
              <Col xs={5}>
                <h5 style={{ fontWeight: '700' }}>Event</h5>
              </Col>
              <Col xs={7}>
                <h5>Performance</h5>
              </Col>
                        ...
                        >
                          <Row>
                            <div>
                              ...
                              <div className='EventItemRightDivIcons'>
                                {' '}
                                <img
                                  className='EventReportCircle'
                                  src={require('../../assets/images/greyIcon.png')}
