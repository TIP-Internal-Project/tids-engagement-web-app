  ...
  const actualBudget = rows.reduce((acc, row) => acc + row.cip, 0)
    ...
    useEffect(() => {
      ...
      const sortedByCreationDate = activeEvents.sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime())
    ...
    const renderedEvents = Object.values(sortedEvents).filter(event => event.status === 'Active' || event.status === 'Inactive' || event.status === 'Completed' || event.status !== 'Archived').map((event, index) => {
      ...
      {selectedEvent && (
        <>
          ...
          <Col md={3} style={ExpensePanelsBorders}>
            <p style={eventOverviewBudget}>Estimated Budget: {selectedEvent.estimatedBudget}</p>
            <p style={eventOverviewBudget}>Actual Budget: {actualBudget}</p>
            <p style={{ fontFamily: 'Mulish', fontSize: '16px', paddingTop: '12px', paddingLeft: '18px', width: 'auto' }}>
              Budget Utilization: 
              {selectedEvent.estimatedBudget > 0 ? 
                `${((actualBudget / selectedEvent.estimatedBudget) * 100).toFixed(2)}%` : 
                'N/A'}
  </p>
            <ActualAndEstimatedBudgetChart actualBudget={actualBudget} estimatedBudget={selectedEvent.estimatedBudget} />
          </Col>
          ...
          <Col md={9}> 
            ...
            <TableContainer component={Paper} style={ExpensePanelsBorders}>
              <Table sx={{ minWidth: 650 }} size="medium" aria-label="a dense table">
                ...
                <TableBody>
                  {rows.map((row) => (
                    <TableRow key={row.cip} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                      <TableCell component="th" scope="row">{row.cip}</TableCell>
                      ...
                      <TableCell align="center">
                        <p>{row.description}</p>
                          ...
                          <ButtonBase onClick={handleClickOpen}>
                                  ...
                                  <Image src={require('../assets/images/receiptSample.png')} alt="thumbnail" />
                                </ButtonBase>
                                <Dialog open={open} onClose={handleClose}>
                                 
                                  <DialogImage src={require('../assets/images/receiptSample.png')} alt="full size" />
                                </Dialog>
                      </TableCell>
                    </TableRow>
                  ))}
