...
const ExpenseModal: React.FC<ExpenseModalProps> = ({ show, onHide, event, action }) => {
  ...
  const addExpense = () => {
    ...
    if (modifiedExpenseIndex !== -1) {
      ...
      while (exist) {
        const idExists =
          expenses.filter((expense) => expense.id === `${event.eventId}-${expenses.length + count}`)
            .length > 0
              ...
              <Row className='mb-50'>
                ...
                <Col xs={4}>
                  <Form.Group className='mb-3'>
                    ...
                    />
                    <br />
                    <label className='upload-receipt-label' htmlFor='imageInput'>
                      <img
                        id='imagePreview'
                ...
                <Col xs={4}>
                  <Form.Group>
                    ...
                    <Form.Control
                      ...
                      onChange={(e) => {
                        ...
                        if (
                          decimalCount > 1 ||
                          (decimalCount === 1 && amount.split('.')[1].length > 2)
                        ) {
                          amount = newExpenseAmount
                        }
                  ...
                  <div className='d-flex justify-content-center align-items-center py-3'>
                    {isModifying ? (
                      <Button variant='success' className='save-width' onClick={addExpense}>
                        Save
                      </Button>
                    ) : (
                      <Button variant='success' className='save-width' onClick={addExpense}>
                  ...
                  <tbody>
                    {expenses.map((expense) => (
                      <tr
                        key={expense.id}
                        className={
                          selectedRowId === expense.id ? 'row-clicked-lightgray' : 'row-clicked-inherit'
                        }
